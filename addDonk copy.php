...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    if ($isValid) {
        ...
        $info = !empty(trim($_POST['donkey_info'])) ? trim($_POST['donkey_info']) : null;
        $sex = !empty($_POST['sex']) ? $_POST['sex'] : null;
        $status = isset($_POST['isActive']) ? 1 : 0;
        $bday = $_POST['bDay'];

        $task ="INSERT INTO donkeys (`name`, `bday`, `sex`, `color`, `guardian`, `info`, `status`)
                VALUES (?, ?, ?, ?, ?, ?, ?)";
        $query = $conn -> prepare($task);
        $result = 0;
        if($query){
            $query -> bind_param("sssiisi", 
            $name, $bday, $sex, $colorId, $guardianId, $info, $status
            );
        ...
        $idDonk = FirstFreeID('donkeysdb', 'donkeys');
        $imgToTask = '';
        $profilePic = saveFile('profilePic', 'png, jpg, webp, gif', 10, "img/donkeys/$idDonk/prof");
        if($profilePic[0] == 1){
            $imgToTask .= "('$profilePic[1]', $idDonk, NULL),";
        }
        if(is_uploaded_file($_FILES['extraPics']['tmp_name'])){
            $extraPics = saveFile('profilePic', 'png, jpg, webp, gif', 10, "img/donkeys/$idDonk");
            if($extraPics[0] == 1){
        ...
        $imgToTask = substr($imgToTask, 0, -1);
        if(strlen($imgToTask) > 0){
            $taskImg = "INSERT INTO img VALUES $imgToTask";
            $queryimg = mysqli_query($conn, $taskImg);
        }
              
    }
    header("Location: addDonk.php?result=$result");
    exit;
}
?>
<!DOCTYPE html>
                    ...
                    <article class="card h-100 shadow-sm border-0 position-relative">
                        ...
                        <form class="row g-3 needs-validation" action="addDonk.php" method="POST" enctype="multipart/form-data" novalidate>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" id="donkeyName" name="donkeyName" required>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="date" class="form-control" id="bDay" name="bDay" max="<?= date('Y-m-d'); ?>" required>
                                <div class="invalid-feedback">
                            ...
                            <div class="col-md-6 d-flex align-items-center">
                                <div>
                                    ...
                                    <input type="radio" class="btn-check" name="sex" id="fem" value="F" autocomplete="off" required>
                                    <label class="btn btn-outline-danger" for="fem">Samica</label>

                                    <input type="radio" class="btn-check" name="sex" id="M" value="samiec" autocomplete="off" required>
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control" id="donkeyInfo" name="donkeyInfo" rows="4" placeholder="Wpisz dodatkowe informacje, cechy charakteru, przebyte choroby, czy uwagi dotyczące osiołka..."></textarea>
                                <div class="invalid-feedback">
